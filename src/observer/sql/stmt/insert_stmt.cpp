...
RC InsertStmt::check_incomplete_rows(
    BaseTable *table, const InsertSqlNode &inserts, std::vector<std::vector<Value>> &rows)
{
  RC rc = RC::SUCCESS;
  ...
  for (const std::vector<Value> &values : inserts.values) {
    ...
    if (value_num != inserts.attrs_name.size()) {
      LOG_WARN("value mismatch with attr_names. value num=%d, attr_names num=%d", value_num,
                     inserts.attrs_name.size());
      return RC::INVALID_ARGUMENT;
    }
