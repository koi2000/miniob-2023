...
DateTime::DateTime(std::string& xml_str) {
    tm tmp;
    sscanf(xml_str.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ", &tmp.tm_year, &tmp.tm_mon, &tmp.tm_mday, &tmp.tm_hour,
...
bool DateTime::is_valid_xml_datetime(const std::string& str) {
    ...
    tm tmp;
    int ret = sscanf(str.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ", &tmp.tm_year, &tmp.tm_mon, &tmp.tm_mday,
