...
RC MysqlCommunicator::read_event(SessionEvent*& event) {
    ...
    if (ret != 0) {
        LOG_WARN("failed to read packet payload. length=%d, addr=%s, error=%s", packet_header.payload_length,
                 addr_.c_str(), strerror(errno));
        return RC::IOERR_READ;
    }
    ...
    if (command_type == 0x03) {  // COM_QUERY，这是一个普通的文本请求
        ...
        if (rc != RC::SUCCESS) {
            LOG_WARN("failed to decode query packet. packet length=%ld, addr=%s, error=%s", buf.size(), addr(),
                     strrc(rc));
            return rc;
        }
