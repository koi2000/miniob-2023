...
class BenchmarkBase : public Fixture {
  public:
    ...
    virtual void SetUp(const State& state) {
        ...
        if (rc != RC::SUCCESS) {
            LOG_WARN("failed to create record buffer pool file. filename=%s, rc=%s", record_filename.c_str(),
                     strrc(rc));
        ...
        LOG_INFO("test %s setup done. threads=%d, thread index=%d", this->Name().c_str(), state.threads(),
                 state.thread_index());
    }
    ...
    virtual void TearDown(const State& state) {
        ...
        LOG_INFO("test %s teardown done. threads=%d, thread index=%d", this->Name().c_str(), state.threads(),
                 state.thread_index());
    }
    ...
    void FillUp(int32_t min, int32_t max, vector<RID>& rids) {
        ...
        for (int32_t record_value : record_values) {
            ...
            [[maybe_unused]] RC rc =
                handler_.insert_record(reinterpret_cast<const char*>(&record), sizeof(record), &rid);
