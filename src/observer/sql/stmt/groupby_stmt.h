...
public:
  std::vector<std::unique_ptr<AggrFuncExpr>> &get_agg_exprs() { return agg_exprs_; }
  std::vector<std::unique_ptr<FieldExpr>>    &get_field_exprs() { return field_exprs_; }
  std::vector<std::unique_ptr<Expression>>   &get_groupby_fields() { return groupby_fields_; }
  void set_agg_exprs(std::vector<std::unique_ptr<AggrFuncExpr>> &&agg_exprs) { agg_exprs_ = std::move(agg_exprs); }
  void set_field_exprs(std::vector<std::unique_ptr<FieldExpr>> &&field_exprs) { field_exprs_ = std::move(field_exprs); }
