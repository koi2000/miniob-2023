...
class Log {
  ...
  private:
    ...
    template <class T> int out(const LOG_LEVEL console_level, const LOG_LEVEL log_level, T& message);
...
#define LOG_HEAD(prefix, level)                                                                                        \
    if (common::g_log) {                                                                                               \
        ...
        if (p) {                                                                                                       \
            ...
            snprintf(sz_head, LOG_HEAD_SIZE, "%04d-%02d-%02d %02d:%02d:%02u.%06d pid:%u tid:%llx ctx:%lx",             \
                     p->tm_year + 1900, p->tm_mon + 1, p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec, usec,             \
                     (int32_t)getpid(), gettid(), common::g_log->context_id());                                        \
            common::g_log->rotate(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);                                       \
        }                                                                                                              \
        snprintf(prefix, sizeof(prefix), "[%s %s %s@%s:%u] >> ", sz_head, (common::g_log)->prefix_msg(level),          \
                 __FUNCTION__, __FILE_NAME__, (int32_t)__LINE__);                                                      \
    }
...
template <class T> int Log::out(const LOG_LEVEL console_level, const LOG_LEVEL log_level, T& msg) {
    bool locked = false;
