...
std::string SedaConfig::get_thread_pool(std::string& stage_name) {
    ...
    if (thread_name.empty()) {
        LOG_ERROR("Failed to set %s of the %s, use the default threadpool %s", thread_pool_id.c_str(),
...
SedaConfig::status_t SedaConfig::gen_next_stages() {
    try {
        for (std::vector<std::string>::iterator it_name = stage_names_.begin(); it_name != stage_names_.end();
             it_name++) {
