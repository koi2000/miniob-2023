...
RC SelectStmt::process_from_clause(Db* db,
    ...
    for (size_t i = 0; i < from_relations.size(); ++i) {
        ...
        if (RC::SUCCESS != rc) {
            LOG_WARN("Create SelectStmt: From Clause: Process Base Relation %s Failed!",
                     relations.base_relation.first.c_str());
            return rc;
        }
...
RC SelectStmt::create(Db* db,
    ...
    auto check_project_expr = [&table_map, &tables, &default_table, &table_alias_map,
                               &has_aggr_func_expr](Expression* expr) {
    ...
    for (size_t i = 0; i < select_sql.project_exprs.size(); ++i) {
        ...
        if (expr->type() == ExprType::FIELD) {
            ...
            if ((0 == strcmp(table_name, "*")) && (0 == strcmp(field_name, "*"))) {  // * or *.*
                ...
                if (rc = field_expr->check_field(table_map, tables, default_table, table_alias_map);
                    rc != RC::SUCCESS) {
    ...
    if (has_aggr_func_expr || select_sql.groupby_exprs.size() > 0) {
        ...
        rc = GroupByStmt::create(db, default_table, &table_map, select_sql.groupby_exprs, groupby_stmt,
                                 std::move(aggr_exprs), std::move(field_exprs));
        if (rc != RC::SUCCESS) {
    ...
    if (select_sql.orderbys.size() > 0) {
        ...
        rc = OrderByStmt::create(db, default_table, &table_map, select_sql.orderbys, orderby_stmt,
