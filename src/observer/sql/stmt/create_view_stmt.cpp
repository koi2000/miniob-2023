...
CreateViewStmt::CreateViewStmt(bool allow_write, const std::string view_name,
    const std::vector<AttrInfoSqlNode> attr_infos, std::vector<Field> map_fields, SelectSqlNode *select_sql)
    : view_name_(std::move(view_name)),
      allow_write_(allow_write),
      attr_infos_(std::move(attr_infos)),
      map_fields_(std::move(map_fields))
{
...
RC CreateViewStmt::create(Db *db, const CreateViewSqlNode &create_view, Stmt *&stmt, SelectSqlNode &select_sql)
{
  ...
  stmt = new CreateViewStmt(
      allow_write, std::move(create_view.view_name), std::move(attr_infos), std::move(map_fields), &sql_back);
