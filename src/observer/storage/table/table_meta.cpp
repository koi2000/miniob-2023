...
RC TableMeta::init(int32_t table_id, const char* name, int field_num, const AttrInfoSqlNode attributes[]) {
    ...
    if (trx_fields != nullptr) {
        ...
        for (size_t i = 0; i < trx_fields->size(); i++) {
            ...
            fields_[i] =
                FieldMeta(field_meta.name(), field_meta.type(), field_offset, field_meta.len(), false /*visible*/, field_meta.allow_null());
    ...
    for (int i = 0; i < field_num; i++) {
        ...
        rc = fields_[i + trx_field_num].init(attr_info.name.c_str(), attr_info.type, field_offset, attr_info.length,
                                             true /*visible*/,attr_info.allowNull);
        if (rc != RC::SUCCESS) {
...
const IndexMeta* TableMeta::find_index_by_fields(std::vector<string> index_fields) const {
    int attribute_num = index_fields.size();
    for (const IndexMeta& index : indexes_) {
        ...
        for (int i = 0; i < attribute_num; i++) {
            if (0 != strcmp(fields[i].c_str(), index_fields[i].c_str())) {
                is_same = false;
                break;
            }
        }
