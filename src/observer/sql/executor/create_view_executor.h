...
class CreateViewExecutor {
  public:
    ...
    RC execute(SQLStageEvent* sql_event) {
        ...
        rc = session->get_current_db()->create_view(
            create_view_stmt->view_name().c_str(), create_view_stmt->allow_write(), create_view_stmt->attr_infos(),
            create_view_stmt->map_fields(), create_view_stmt->get_select_sql_node());
        if (RC::SUCCESS != rc) {
