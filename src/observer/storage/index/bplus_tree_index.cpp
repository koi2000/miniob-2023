...
RC BplusTreeIndex::create(const char* file_name,
    ...
    std::vector<int> attrtlens;
    for (FieldMeta field_meta : field_metas) {
        meta_len += field_meta.len();
        attrtypes.push_back(field_meta.type());
        attrtlens.push_back(field_meta.len());
    }

    RC rc = index_handler_.create(file_name, attrtypes, attrtlens);
    // if (RC::SUCCESS != rc) {
...
RC BplusTreeIndex::open(const char* file_name, const IndexMeta& index_meta, std::vector<FieldMeta> field_metas) {
    ...
    if (RC::SUCCESS != rc) {
        // LOG_WARN("Failed to open index_handler, file_name:%s, index:%s, field:%s, rc:%s", file_name,
        // index_meta.name(),
...
RC BplusTreeIndex::insert_entry(const char* record, const RID* rid) {
    ...
    for (FieldMeta field_meta : field_metas_) {
        offsets.push_back(field_meta.offset());
    }
...
RC BplusTreeIndexScanner::open(const char* left_key,
                               int left_len,
                               bool left_inclusive,
                               const char* right_key,
                               int right_len,
