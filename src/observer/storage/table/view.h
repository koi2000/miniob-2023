...
public:
  ...
  RC   create(int32_t table_id, bool allow_write,
        ...
        const std::vector<AttrInfoSqlNode> &attr_infos, const std::vector<Field> &map_fields, SelectSqlNode *select_sql);
  RC   open();
  RC   drop();
...
private:
  ...
  // view列映射的原始表中的列，可写view中所有col映射的都是Field，只读view不需要用到这个映射
  std::vector<Field> map_fields_;
