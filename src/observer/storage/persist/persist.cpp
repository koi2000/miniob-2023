...
RC PersistHandler::write_file(int size, const char *data, int64_t *out_size)
{
  ...
  if (file_name_.empty()) {
    ...
    if ((write_size = write(file_desc_, data, size)) != size) {
      LOG_ERROR("Failed to write %d:%s due to %s. Write size: %lld", file_desc_, file_name_.c_str(),
...
RC PersistHandler::write_at(uint64_t offset, int size, const char *data, int64_t *out_size)
{
  ...
  if (file_name_.empty()) {
    ...
    if (lseek(file_desc_, offset, SEEK_SET) == off_t(-1)) {
      ...
      int64_t write_size = 0;
      if ((write_size = write(file_desc_, data, size)) != size) {
        LOG_ERROR("Failed to write %llu of %d:%s due to %s. Write size: %lld", offset, file_desc_,
  ...
  if (file_name_.empty()) {
    ...
    if (lseek(file_desc_, offset, SEEK_SET) == off_t(-1)) {
      ...
      if (read_size == 0) {
        ...
        LOG_WARN("failed to read file. file name=%s, offset=%lld, size=%d, error=%s", file_name_.c_str(),
