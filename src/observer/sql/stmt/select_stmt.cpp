...
RC SelectStmt::process_from_clause(Db *db, std::vector<BaseTable *> &tables, std::vector<std::string> &alias,
    ...
    const std::unordered_map<std::string, BaseTable *> &parent_table_map)
{
  ...
  if (has_aggr_func_expr || select_sql.groupby_exprs.size() > 0) {
    ...
    if (select_sql.having_conditions != nullptr) {
      rc = FilterStmt::create(db, default_table, &table_map, tables, select_sql.having_conditions, having_filter_stmt);
      if (rc != RC::SUCCESS) {
        LOG_WARN("cannot construct filter stmt");
        return rc;
      }
