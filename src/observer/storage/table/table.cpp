...
RC Table::open(const char* meta_file, const char* base_dir) {
    ...
    for (int i = 0; i < index_num; i++) {
        ...
        std::vector<std::string> field_names = index_meta->fields();
...
RC Table::create_index(Trx* trx, std::vector<FieldMeta> field_metas, const char* index_name) {
    ...
    if (rc != RC::SUCCESS) {
        LOG_INFO("Failed to init IndexMeta in table:%s, index_name:%s, field_name_size:%s", name(), index_name,
                 field_metas.size());
        return rc;
    }
