...
template <class T> class MemPool {
  public:
     ...
     */
    virtual int
    init(bool dynamic = true, int pool_num = DEFAULT_POOL_NUM, int item_num_per_pool = DEFAULT_ITEM_NUM_PER_POOL) = 0;
...
template <class T> int MemPoolSimple<T>::init(bool dynamic, int pool_num, int item_num_per_pool) {
    if (pools.empty() == false) {
        LOG_WARN("Memory pool has been initialized, but still begin to be initialized, this->name:%s.",
                 this->name.c_str());
        return 0;
    }

    if (pool_num <= 0 || item_num_per_pool <= 0) {
        LOG_ERROR("Invalid arguments,  pool_num:%d, item_num_per_pool:%d, this->name:%s.", pool_num, item_num_per_pool,
                  this->name.c_str());
        return -1;
    }
    ...
    LOG_INFO("Extend one pool, this->size:%d, item_num_per_pool:%d, this->name:%s.", this->size, item_num_per_pool,
             this->name.c_str());
    return 0;
}
...
template <class T> int MemPoolSimple<T>::extend() {
    ...
    if (pool == nullptr) {
        MUTEX_UNLOCK(&this->mutex);
        LOG_ERROR("Failed to extend memory pool, this->size:%d, item_num_per_pool:%d, this->name:%s.", this->size,
                  item_num_per_pool, this->name.c_str());
        return -1;
    }
