...
RC Table::sync() {
    ...
    for (Index* index : indexes_) {
        rc = index->sync();
        if (rc != RC::SUCCESS) {
            LOG_ERROR("Failed to flush index's pages. table=%s, index=%s, rc=%d:%s", name(), index->index_meta().name(),
                      rc, strrc(rc));
            return rc;
        }
    }
...
RC Table::update_record(Record& record, const std::vector<std::string>& attr_names, const std::vector<Value*>& values) {
    ...
    for (size_t c_idx = 0; c_idx < attr_names.size(); c_idx++) {
        ...
        for (int i = 0; i < user_field_num; ++i) {
            ...
            if (value->is_null() && field_meta->nullable()) {
                ...
                LOG_WARN("field type mismatch. table=%s, field=%s, field type=%d, value_type=%d", name(),
                         field_meta->name(), attr_type, value_type);
                return RC::SCHEMA_FIELD_TYPE_MISMATCH;
            }
    ...
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to delete indexes of record (rid=%d.%d). rc=%d:%s", record.rid().page_num,
                  record.rid().slot_num, rc, strrc(rc));
        return rc;
    }
    ...
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to update record (rid=%d.%d). rc=%d:%s", record.rid().page_num, record.rid().slot_num, rc,
                  strrc(rc));
        return rc;
    }
...
RC Table::update_record(Record& old_record, Record& new_record) {
    ...
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to delete indexes of record (rid=%d.%d). rc=%d:%s", old_record.rid().page_num,
                  old_record.rid().slot_num, rc, strrc(rc));
        return rc;
    }
    ...
    if (rc != RC::SUCCESS) {
        ...
        LOG_ERROR("Failed to update record (rid=%d.%d). rc=%d:%s", new_record.rid().page_num, new_record.rid().slot_num,
                  rc, strrc(rc));
        return rc;
    }
